...
/**
 * This script is idempotent, so there is no risk in running it multiple times,
 * even if real translations already exist
 */
async function main() {
   ...
   */
  let all = new Set(
    (await globby(['**/*.yaml', '**/*.yml'], { cwd: TRANSLATIONS })).map((filePath) =>
